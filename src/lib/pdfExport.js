 ...
 * Dark header band with EWC logo, light body for print readability.
 */
...
function buildReportHTML({ inputs, results, leadDetails }) {
  ...
  return `
    <div style="font-family: 'Manrope', sans-serif; color: #1A1F1B;">
      ...
      <div style="background: #0A0A0B; padding: 24px 40px; display: flex; align-items: center; gap: 16px;">
        <img src="/ELITE_LOGO.png" alt="EWC" style="width: 56px; height: 56px; flex-shrink: 0;" crossorigin="anonymous" />
        <div style="flex: 1;">
          <div style="font-size: 10px; text-transform: uppercase; letter-spacing: 2px; color: #4FA8F0; font-weight: 600; margin-bottom: 4px;">
            Elite Wealth Creators
          </div>
          <h1 style="font-family: 'Fraunces', serif; font-size: 26px; font-weight: 500; margin: 0; color: #FFFFFF; line-height: 1;">
            SMSF Property Investment Report
          </h1>
          <p style="margin: 4px 0 0 0; font-size: 11px; color: #9ca0a8; font-style: italic;">
            Your future built on property
          </p>
        </div>
        <div style="text-align: right; font-size: 11px; color: #9ca0a8;">
          <div>${today}</div>
          ${leadDetails && leadDetails.name ? `<div style="font-weight: 600; color: #FFFFFF; margin-top: 2px;">For ${leadDetails.name}</div>` : ""}
        </div>
      </div>

      <div style="padding: 32px 40px;">
        <!-- Property summary -->
        <div style="background: #F4F6F8; padding: 16px; margin-bottom: 24px; border-left: 3px solid #2b8fe0; border-radius: 2px;">
          <div style="font-size: 10px; text-transform: uppercase; letter-spacing: 1.5px; color: #6B6F76; font-weight: 600; margin-bottom: 6px;">
            Property
          </div>
          <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; font-size: 12px;">
            <div>
              <div style="color: #6B6F76; margin-bottom: 2px;">Price</div>
              <div style="font-weight: 600; font-size: 14px;">${fmtCurrency(inputs.propertyPrice)}</div>
            </div>
            <div>
              <div style="color: #6B6F76; margin-bottom: 2px;">Location</div>
              <div style="font-weight: 600; font-size: 14px;">${inputs.state}</div>
            </div>
            <div>
              <div style="color: #6B6F76; margin-bottom: 2px;">Holding period</div>
              <div style="font-weight: 600; font-size: 14px;">${inputs.holdingYears} years</div>
            </div>
            <div>
              <div style="color: #6B6F76; margin-bottom: 2px;">SMSF stage</div>
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 24px;">
          ...
          ${kpiCardHTML("Weekly " + (cashFlowIsNegative ? "Out-of-Pocket" : "Surplus"), fmtCurrency(Math.abs(results.weeklyAfterTax), 2), cashFlowIsNegative ? "Negative gearing" : "Positive geared", cashFlowIsNegative ? "#C95B3A" : "#2D8F4F")}
          ${kpiCardHTML("After-tax Profit", fmtCurrencyShort(results.afterTaxProfit), `Sale ${fmtCurrencyShort(results.propertySoldPrice)}`, results.afterTaxProfit > 0 ? "#2D8F4F" : "#C95B3A")}
        ...
        <div style="margin-bottom: 24px;">
          ...
          <table style="width: 100%; font-size: 10px; border-collapse: collapse;">
            <thead>
              <tr style="border-bottom: 1.5px solid #2b8fe0;">
                <th style="text-align: left; padding: 6px 4px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; color: #6B6F76; font-size: 9px;">Year</th>
                <th style="text-align: right; padding: 6px 4px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; color: #6B6F76; font-size: 9px;">Property</th>
                <th style="text-align: right; padding: 6px 4px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; color: #6B6F76; font-size: 9px;">Gross Rent</th>
                <th style="text-align: right; padding: 6px 4px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; color: #6B6F76; font-size: 9px;">Outgoings</th>
                <th style="text-align: right; padding: 6px 4px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; color: #6B6F76; font-size: 9px;">After-tax CF</th>
                <th style="text-align: right; padding: 6px 4px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; color: #6B6F76; font-size: 9px;">Equity</th>
              </tr>
            </thead>
            <tbody>
              ${results.projection
                ...
                <tr style="border-bottom: 1px solid #E7E2D6;">
                  ...
                  <td style="text-align: right; padding: 5px 4px; color: #6B6F76;">${fmtCurrency(y.outgoings)}</td>
                  <td style="text-align: right; padding: 5px 4px; font-weight: 600; color: ${y.afterTaxCF >= 0 ? "#2D8F4F" : "#C95B3A"};">${fmtCurrency(y.afterTaxCF)}</td>
        ...
        <!-- Disclaimer -->
        <div style="border-top: 1px solid #D6D1C4; padding-top: 12px; margin-top: 16px; font-size: 9px; color: #6B6F76; line-height: 1.5;">
      ...
      <!-- Dark footer -->
      <div style="background: #0A0A0B; padding: 12px 40px; text-align: center;">
        <p style="margin: 0; color: #4FA8F0; font-weight: 600; font-size: 10px; letter-spacing: 1px; text-transform: uppercase;">
          Elite Wealth Creators · Australian SMSF Property Specialists
        </p>
        <p style="margin: 4px 0 0 0; color: #9ca0a8; font-size: 9px;">
          elitewealthcreators.com
        </p>
      </div>
    </div>
  `;
...
function kpiCardHTML(label, value, sub, accentColor = "#1A1F1B") {
  return `
    <div style="border: 1px solid #D6D1C4; padding: 12px; border-radius: 2px;">
      <div style="font-size: 9px; text-transform: uppercase; letter-spacing: 1px; color: #6B6F76; font-weight: 600; margin-bottom: 6px;">${label}</div>
      ...
      <div style="font-size: 10px; color: #6B6F76;">${sub}</div>
    </div>
  `;
}
...
function sectionHTML(title, body) {
  return `
    <div style="margin-bottom: 20px;">
      <div style="font-family: 'Fraunces', serif; font-size: 14px; font-weight: 500; margin-bottom: 8px; color: #1A1F1B; padding-bottom: 4px; border-bottom: 1.5px solid #2b8fe0;">${title}</div>
      ${body}
    </div>
  `;
}
...
function rowHTML(label, value, accent = "default", bold = false) {
  ...
  return `
    <div style="display: flex; justify-content: space-between; padding: 4px 0; font-size: 11.5px; border-bottom: 1px solid #F4F6F8;">
      <span style="color: ${bold ? "#1A1F1B" : "#6B6F76"}; font-weight: ${weight};">${label}</span>
